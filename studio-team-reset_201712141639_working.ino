...
int redLed = D5;  // On for a period of time after a reset to indicate success.
// Push Buttons
int buttonOne = D2;
int buttonTwo = D3;
...
void loop() {
    ...
    if ((digitalRead(buttonOne) == 0) && (digitalRead(buttonTwo) == 0)) {  // Enter if both buttons pressed
        ...
        if (((Time.now() - lastButtonsPressed) > buttonDelay) && !resetSent) {
            Particle.publish("Studio", "reset");  // Send the reset command after the buttonDelay
...
void PingHandler(const char *event, const char *data) {
    ...
    if (String(data) == "got_reset") {
        digitalWrite(redLed, 1);  // turn on red LED when Driver gets reset
    }
    if (String(data) == "reset_done") {
        digitalWrite(redLed, 0);  // turn off red LED after reset
    }
}
